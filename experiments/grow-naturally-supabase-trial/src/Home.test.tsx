...
describe("Home", () => {
  ...
  it("shows the initial four module positions", () => {
    ...
    expect(screen.getByLabelText("未来功能预留")).toBeInTheDocument();
  });
  ...
  it("opens area branches from the home page without leaving first", async () => {
    ...
    expect(screen.getByRole("link", { name: "酒水库" })).toBeInTheDocument();
  });
  ...
  it("opens the new chiller, dry store, and drinks inventory lists from area branches", async () => {
    ...
    expect(screen.getByRole("table", { name: "酒水库库存总列表" })).toBeInTheDocument();
  });
  ...
  it("hides home search results after clicking outside the search area", async () => {
    ...
    await user.click(screen.getByLabelText("未来功能预留"));
