...
export function PurchasingPage() {
  ...
  const matchingItem =
    state.kind === "review" ? state.items.find((item) => item.clientId === matchingClientId) ?? null : null;
  ...
  return (
    <main className="purchasing-page-shell">
        ...
        {state.kind === "review" && (
          <section className="purchase-review" aria-labelledby="purchase-review-title">
            <header className="purchase-review-heading">
              ...
              <button className="purchasing-source-button" onClick={() => setShowSource(true)} type="button">
      ...
      {matchingItem && <ProductMatchDialog itemName={matchingItem.product_name} onChoose={chooseProduct} onClose={() => setMatchingClientId(null)} selectedProductId={matchingItem.supplierProductId} />}
