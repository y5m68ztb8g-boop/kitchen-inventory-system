...
test.beforeEach(async ({ request }) => {
  const response = await request.post("/api/inventory-db", { data: emptyInventoryDatabase });
...
test("home screen shows the four initial modules", async ({ page }) => {
  ...
  await expect(page.getByRole("link", { name: "AI录入" })).toHaveAttribute("href", "#purchasing");
