...
export function Home() {
  ...
  const wineCellarValue = wineCellarSnapshot ? calculateInventorySummary(wineCellarSnapshot).totalValue : 0;
  const totalInventoryValue = freezerInventoryValue + dryStoreInventoryValue + wineCellarValue;
  ...
  const inventorySearchItems = [
    ...
    ...(wineCellarSnapshot
      ? wineCellarSnapshot.positions
          .filter((position) => position.active)
          .map((position) => {
            const assignment = wineCellarSnapshot.assignments.find((item) => item.positionId === position.id && item.active);
            const rack = wineCellarSnapshot.racks.find((item) => item.id === position.rackId && item.active);
            return {
              id: position.id,
              productName: assignment?.productName || "未指定酒品",
              productNameEn: assignment?.productName || "Unassigned drinks product",
              warehouse: "酒水库",
              warehouseEn: "Drinks",
              rack: rack?.name || "未分配酒架",
              rackEn: rack?.name || "Unassigned rack",
              position: position.code,
              positionEn: position.code,
              quantityText: `${position.currentQuantity} ${position.stockUnit}`,
              quantityTextEn: `${position.currentQuantity} ${position.stockUnit}`
            } satisfies StockItem;
          })
      : [])
  ];
