...
describe("OrderingPage", () => {
  beforeEach(() => {
    ...
    sendSupplierEmail.mockResolvedValue({ status: "delivered" });
  ...
  it("keeps the email draft editable and sends via sendSupplierEmail with success status", async () => {
    const user = userEvent.setup();
    prepareSupplierGroup.mockResolvedValue({
      kind: "email-draft",
      draft: {
    ...
    const sendButton = within(dialog).getByRole("button", { name: "发送邮件" });
    await user.click(sendButton);

    expect(sendSupplierEmail).toHaveBeenCalledWith(emptyBatch.id, "CMP", expect.objectContaining({
      supplierCode: "CMP",
      to: "orders@campbells.example",
      subject: "Purchase order PO-5005",
      body: "Please prepare the revised order."
    }));
    expect(await screen.findByText(/发送成功|已发送|delivered/)).toBeInTheDocument();
  });

  it("opens Brakes manual assistant and shows BRK queue items with copy actions", async () => {
    const user = userEvent.setup();
    getCurrentOrderingBatch.mockResolvedValue({ batch: task7Batch, readyIntakes: [] });
    render(<OrderingPage />);

    await user.click(await screen.findByRole("button", { name: "Brakes 人工 Quick Add" }));
    const dialog = await screen.findByRole("dialog", { name: "Brakes 人工 Quick Add 助手" });

    expect(dialog).toHaveTextContent("Brakes The Juice Orange");
    expect(dialog).toHaveTextContent("JUICE-12 × 2");
    expect(within(dialog).getByRole("button", { name: `复制编码 ${task7Batch.items[2].supplierProductCode}` })).toBeInTheDocument();
    expect(within(dialog).getByRole("button", { name: "复制数量 2" })).toBeInTheDocument();
    expect(runBrakesQuickAdd).not.toHaveBeenCalled();
  });

  it("opens the manual helper without deduplicating Brakes rows by status", async () => {
    const user = userEvent.setup();
    const retryBatch = {
      ...task7Batch,
      items: [
        ...
        { ...task7Batch.items[2], id: "item-brakes-failed", productName: "Brakes Failed Item", supplierProductCode: "FAILED-2", orderQuantity: 10, brakesStatus: "Failed" }
      ]
    };
    ...
    expect(dialog).toHaveTextContent("Brakes The Juice Orange");
    expect(dialog).toHaveTextContent("Brakes Failed Item");
    expect(dialog).toHaveTextContent("JUICE-12 × 2");
    expect(dialog).toHaveTextContent("FAILED-2 × 10");
    expect(within(dialog).getByRole("button", { name: "复制编码 JUICE-12" })).toBeInTheDocument();
    expect(within(dialog).getByRole("button", { name: "复制数量 10" })).toBeInTheDocument();
    expect(runBrakesQuickAdd).not.toHaveBeenCalled();
  });

  it.each([
    { actionLabel: "BRK 可录入", batch: task7Batch },
    {
      actionLabel: "BRK 无可录入",
      batch: {
        ...task7Batch,
        items: task7Batch.items.filter((item) => item.supplierGroup !== "BRK")
      }
    }
  ])("opens Brakes manual assistant dialog for $actionLabel batches", async ({ batch }) => {
    const user = userEvent.setup();
    ...
    if (batch.items.some((item) => item.supplierGroup === "BRK")) {
      expect(dialog).toHaveTextContent("Brakes The Juice Orange");
      expect(dialog).not.toHaveTextContent("当前没有可录入的 Brakes 商品。");
      expect(within(dialog).getByRole("button", { name: "复制编码 JUICE-12" })).toBeInTheDocument();
    } else {
      expect(dialog).toHaveTextContent("当前没有可录入的 Brakes 商品。");
      expect(dialog.querySelectorAll("button[aria-label^='复制编码']").length).toBe(0);
    }
    expect(runBrakesQuickAdd).not.toHaveBeenCalled();
  });

  it("opens and closes the Brakes manual assistant dialog", async () => {
    const user = userEvent.setup();
    getCurrentOrderingBatch.mockResolvedValue({ batch: task7Batch, readyIntakes: [] });
    ...
    await user.click(screen.getByRole("button", { name: "关闭 Brakes 人工助手" }));

    expect(screen.queryByRole("dialog", { name: "Brakes 人工 Quick Add 助手" })).not.toBeInTheDocument();
    expect(runBrakesQuickAdd).not.toHaveBeenCalled();
  });
