...
import { BrakesManualAssistantDialog } from "./ordering/BrakesManualAssistantDialog";
import type {
  InventoryReviewItem,
  OrderingProfile,
  PurchaseBatch,
...
export function OrderingPage() {
  ...
  const [brakesHelperOpen, setBrakesHelperOpen] = useState(false);
  ...
  async function sendDraft(draft: SupplierEmailDraft) {
    if (!batch) return;
    await saveDraft(draft);
    const result = await sendSupplierEmail(batch.id, draft.supplierCode, draft);
  ...
  return (
    <main className="ordering-page">
          ...
          return (
            <section className={`ordering-group ordering-group-${group.code.toLowerCase()}`} key={group.code}>
              <header>
                ...
                {group.code === "BRK" && supplier?.status !== "Ordered" && <button className="ordering-prepare-button" onClick={() => setBrakesHelperOpen(true)} type="button">Brakes 人工 Quick Add</button>}
      ...
      {brakesHelperOpen && <BrakesManualAssistantDialog batch={batch} onClose={() => setBrakesHelperOpen(false)} />}
      ...
      {emailDraft && <EmailDraftDialog draft={emailDraft} onChange={setEmailDraft} onClose={() => setEmailDraft(null)} onSave={saveDraft} onSend={sendDraft} supplierName={emailDraft.supplierCode === "CMP" ? "Campbells" : "Mark Murphy"} />}
      {poDialogOpen && (
