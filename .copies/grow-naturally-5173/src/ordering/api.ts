...
const chineseErrorMessages: Record<string, string> = {
  ...
  EMAIL_SERVICE_NOT_CONFIGURED: "邮件服务尚未配置，请先配置发件邮箱。",
  EMAIL_SEND_FAILED: "邮件发送失败，请检查邮箱配置后重试。",
...
export async function sendSupplierEmail(batchId: string, supplierCode: "CMP" | "MM", _draft?: SupplierEmailDraft): Promise<{ batch: PurchaseBatch; status: "delivered" }> {
  const response = await readOrderingResponse<{ batch: PurchaseBatch; status: "delivered" }>(
    `/api/ordering/batches/${encodeURIComponent(batchId)}/suppliers/${supplierCode}/send-email`,
    { method: "POST" }
  );
  return response;
}
...
export type BrakesEdiOrderPreview = {
  supplierCode: "BRK";
  batchId: string;
  poNumber: string;
  lines: Array<{ lineNumber: number; supplierProductCode: string; quantity: number; unit: string }>;
};
...
export function previewBrakesEdiOrder(batchId: string): Promise<BrakesEdiOrderPreview> {
  return readOrderingResponse(`/api/ordering/batches/${encodeURIComponent(batchId)}/suppliers/BRK/edi-preview`, { method: "GET" });
}

export function submitBrakesEdiOrder(batchId: string): Promise<BrakesEdiOrderPreview & { status: string; reference?: string }> {
  return readOrderingResponse(`/api/ordering/batches/${encodeURIComponent(batchId)}/suppliers/BRK/edi-submit`, { method: "POST" });
}
