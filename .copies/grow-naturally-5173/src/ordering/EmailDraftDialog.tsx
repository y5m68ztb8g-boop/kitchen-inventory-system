import { Check, Clipboard, ExternalLink, Mail, X } from "lucide-react";
...
export function EmailDraftDialog({
  ...
  onSend: (draft: SupplierEmailDraft) => Promise<void>;
}) {
  ...
  async function sendMail() {
    ...
    try {
      ...
      setSendError(error instanceof Error ? error.message : "邮件发送失败，请稍后重试。 ");
    } finally {
      setSending(false);
    }
  }
  ...
  return (
    <div className="ordering-dialog-backdrop">
      <section aria-label={`${supplierName} 邮件草稿`} aria-modal="true" className="ordering-dialog email-draft-dialog" role="dialog">
        ...
        {sendError ? <p className="ordering-alert" role="alert">{sendError}</p> : null}
        <div className="ordering-dialog-actions">
          ...
          <button className="ordering-primary" disabled={sending || sent} onClick={() => void sendMail()} type="button">{sent ? <><Check size={18} />已发送</> : sending ? "发送中..." : <><Mail size={18} />发送邮件</>}</button>
        </div>
      </section>
    </div>
  );
