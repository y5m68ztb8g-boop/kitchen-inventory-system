...
type OrderingRouteOptions = {
  ...
  sendEmail?: (input: { supplierCode: string; to: string; subject: string; body: string }) => Promise<{ status: string }>;
  ediGateway?: {
    submit: (input: {
      supplierCode: "BRK";
      poNumber: string;
      lines: Array<{ supplierProductCode: string; quantity: number }>;
...
async function requestJson(url: string, options: RequestInit = {}) {
  ...
  const payload = (await response.json().catch(() => null)) as {
    ...
    reference?: string;
    status?: string;
    result?: { status?: string; reference?: string };
  } | null;
...
describe("ordering routes", () => {
  ...
  it("POST /api/ordering/batches/:batchId/suppliers/:supplierCode/send-email sends draft emails and returns delivered", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    const sendEmail = vi.fn().mockResolvedValue({ status: "delivered" });
    saveBatchPo(database, batchId, "PO-SEND-1");
    addBatchItem(database, {
      batchId,
      productName: "CMP stock item",
      supplierGroup: "CMP",
      supplierProductId: "CMP-100243",
      supplierProductCode: "CMP-100243",
      supplierName: "Campbells",
      packSize: "case",
      orderQuantity: 1,
      orderUnit: "case",
    ...
    saveSupplierEmailDraft(database, {
      batchId,
      supplierCode: "CMP",
      draft: { to: "orders@campbells.example", subject: "PO-5005", body: "Please prepare our Campbells order." }
    });
    ...
    const server = await createTestServer({
      ...
      historicalCandidates: () => orderingTask3Candidates,
      orderingInventory: () => orderingTask3InventorySnapshot,
      sendEmail: sendEmail as (input: { supplierCode: string; to: string; subject: string; body: string }) => Promise<{ status: string }>
    } as unknown as OrderingRouteOptions);
    const baseUrl = await startServer(server);

    const result = await requestJson(`${baseUrl}/api/ordering/batches/${encodeURIComponent(batchId)}/suppliers/CMP/send-email`, {
      method: "POST"
    });

    expect(result.response.status).toBe(200);
    expect(result.payload).toMatchObject({ status: "delivered" });
  ...
  it("POST /api/ordering/batches/:batchId/suppliers/:supplierCode/send-email returns an explicit error when email service is not configured", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    saveBatchPo(database, batchId, "PO-SEND-2");
    saveSupplierEmailDraft(database, {
      batchId,
      supplierCode: "CMP",
      draft: { to: "orders@campbells.example", subject: "PO-5006", body: "Please prepare our Campbells order." }
    });
    ...
    expect(result.response.status).toBeGreaterThanOrEqual(400);
    expect(result.payload).toMatchObject({ error: { code: "EMAIL_SERVICE_NOT_CONFIGURED" } });
  });

  it("GET /api/ordering/batches/:batchId/suppliers/BRK/edi-preview returns a normalized BRK order preview when data is valid", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    saveBatchPo(database, batchId, "PO-BRK-1");
    addBatchItem(database, {
      batchId,
      productName: "Brakes Juice",
      supplierGroup: "BRK",
      supplierProductId: "BRK-100243",
      supplierProductCode: "100243",
      supplierName: "Brakes",
    ...
    addBatchItem(database, {
      batchId,
      productName: "Brakes Tomato",
      supplierGroup: "BRK",
      supplierProductId: "BRK-200101",
    ...
    const result = await requestJson(
      `${baseUrl}/api/ordering/batches/${encodeURIComponent(batchId)}/suppliers/BRK/edi-preview`
    );
    ...
    expect(result.payload).toMatchObject({
      ...
      lines: expect.arrayContaining([
        expect.objectContaining({ supplierProductCode: "100243", quantity: 2 }),
        expect.objectContaining({ supplierProductCode: "200101", quantity: 1 })
      ])
    });
  });

  it("GET /api/ordering/batches/:batchId/suppliers/BRK/edi-preview rejects when PO is missing", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    addBatchItem(database, {
      batchId,
    ...
    expect(result.response.status).toBe(400);
    expect(result.payload).toMatchObject({ error: { code: expect.stringContaining("PO") } });
  });

  it("GET /api/ordering/batches/:batchId/suppliers/BRK/edi-preview rejects missing Brakes item codes", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    saveBatchPo(database, batchId, "PO-BRK-2");
    const item = addBatchItem(database, {
      batchId,
      productName: "Brakes Juice",
      supplierGroup: "BRK",
      supplierProductId: "BRK-100243",
    ...
    database
      .prepare("UPDATE purchase_batch_items SET supplier_product_code = '' WHERE batch_id = ? AND id = ?")
      .run(batchId, item.items[item.items.length - 1].id);
    ...
    expect(result.response.status).toBeGreaterThanOrEqual(400);
    expect(result.payload).toMatchObject({ error: { code: expect.any(String) } });
  });

  it("GET /api/ordering/batches/:batchId/suppliers/BRK/edi-preview rejects invalid Brakes quantities", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    saveBatchPo(database, batchId, "PO-BRK-3");
    const item = addBatchItem(database, {
      batchId,
      productName: "Brakes Juice",
      supplierGroup: "BRK",
    ...
    try {
      database
        .prepare("UPDATE purchase_batch_items SET order_quantity = 0 WHERE batch_id = ? AND id = ?")
        .run(batchId, item.items[item.items.length - 1].id);
    } finally {
      database.pragma("ignore_check_constraints = OFF");
    }
  ...
  it("POST /api/ordering/batches/:batchId/suppliers/BRK/edi-submit returns an explicit error without EDI gateway", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    saveBatchPo(database, batchId, "PO-BRK-4");
    addBatchItem(database, {
      batchId,
      productName: "Brakes Juice",
      supplierGroup: "BRK",
    ...
    const result = await requestJson(
      `${baseUrl}/api/ordering/batches/${encodeURIComponent(batchId)}/suppliers/BRK/edi-submit`,
      { method: "POST" }
    );

    expect(result.response.status).toBeGreaterThanOrEqual(400);
    expect(result.payload).toMatchObject({ error: { code: "EDI_NOT_CONFIGURED" } });
  });

  it("POST /api/ordering/batches/:batchId/suppliers/BRK/edi-submit calls injected EDI gateway and returns the submit result", async () => {
    const database = createPurchasingDatabase(":memory:");
    const batchId = getOrCreateDraftBatch(database).id;
    saveBatchPo(database, batchId, "PO-BRK-5");
    addBatchItem(database, {
      batchId,
      productName: "Brakes Juice",
      supplierGroup: "BRK",
    ...
    const ediGateway = vi
      .fn()
      .mockResolvedValue({ status: "submitted", reference: "BRK-EDI-1" });

    const server = await createTestServer({
      database,
      historicalCandidates: () => orderingTask3Candidates,
      orderingInventory: () => orderingTask3InventorySnapshot,
      ediGateway: { submit: ediGateway }
    });
    const baseUrl = await startServer(server);
    ...
    expect(result.response.status).toBe(200);
    expect(result.payload).toMatchObject({ status: "submitted", reference: "BRK-EDI-1" });
    expect(ediGateway).toHaveBeenCalledWith(
      expect.objectContaining({
        supplierCode: "BRK",
        poNumber: "PO-BRK-5",
        lines: expect.arrayContaining([expect.objectContaining({ supplierProductCode: "100243", quantity: 1 })])
      })
    );
  });
