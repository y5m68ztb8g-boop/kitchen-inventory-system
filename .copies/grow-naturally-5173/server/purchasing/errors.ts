...
const errorDetails: Record<PurchasingApiErrorCode, PurchasingApiErrorDetails> = {
  ...
  EMAIL_SERVICE_NOT_CONFIGURED: { message: "邮件服务尚未配置，请先配置发件邮箱。", status: 503 },
  EMAIL_SEND_FAILED: { message: "邮件发送失败，请检查邮箱配置后重试。", status: 502 },
  EDI_NOT_CONFIGURED: { message: "Brakes EDI 尚未配置，当前不会提交订单。", status: 503 },
  EDI_SUBMISSION_FAILED: { message: "Brakes EDI 提交失败，请稍后重试。", status: 502 }
};
