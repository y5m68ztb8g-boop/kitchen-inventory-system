...
import type { SupplierEmailSender } from "./emailSender";
import { buildBrakesEdiOrder, type BrakesEdiGateway } from "./brakesEdi";
...
export type OrderingRouteOptions = {
  ...
  sendEmail?: SupplierEmailSender;
  emailSender?: SupplierEmailSender;
  ediGateway?: BrakesEdiGateway;
};
...
export function installOrderingRoutes(server: OrderingMiddlewareServer, options: OrderingRouteOptions): void {
  ...
  const sendEmail = options.sendEmail ?? options.emailSender;
  ...
  server.middlewares.use(async (request, response, next) => {
    ...
    try {
      ...
      const sendEmailMatch = url.pathname.match(
        /^\/api\/ordering\/batches\/([^/]+)\/suppliers\/(CMP|MM)\/send-email$/
      );
      if (sendEmailMatch) {
        requireMethod(request, "POST");
        if (!sendEmail) throw new PurchasingApiError("EMAIL_SERVICE_NOT_CONFIGURED");
        const batchId = decodeURIComponent(sendEmailMatch[1]);
        const supplierCode = sendEmailMatch[2] as "CMP" | "MM";
        const batch = getBatchDetail(options.database, batchId);
        const supplier = batch.suppliers.find((entry) => entry.supplierCode === supplierCode);
        if (!supplier?.emailDraft || supplier.status !== "Prepared") {
          throw new PurchasingApiError("SUPPLIER_NOT_PREPARED");
        }
        try {
          const result = await sendEmail({ supplierCode, ...supplier.emailDraft });
          sendJson(response, 200, { batch: await enrichBatch(batch, await orderingInventory()), ...result });
          return;
        } catch {
          throw new PurchasingApiError("EMAIL_SEND_FAILED");
        }
      }

      const ediPreviewMatch = url.pathname.match(
        /^\/api\/ordering\/batches\/([^/]+)\/suppliers\/BRK\/edi-preview$/
      );
      if (ediPreviewMatch) {
        requireMethod(request, "GET");
        const batch = getBatchDetail(options.database, decodeURIComponent(ediPreviewMatch[1]));
        sendJson(response, 200, buildBrakesEdiOrder(batch));
        return;
      }

      const ediSubmitMatch = url.pathname.match(
        /^\/api\/ordering\/batches\/([^/]+)\/suppliers\/BRK\/edi-submit$/
      );
      if (ediSubmitMatch) {
        requireMethod(request, "POST");
        const order = buildBrakesEdiOrder(getBatchDetail(options.database, decodeURIComponent(ediSubmitMatch[1])));
        if (!options.ediGateway) throw new PurchasingApiError("EDI_NOT_CONFIGURED");
        try {
          const result = await options.ediGateway.submit(order);
          sendJson(response, 200, { ...order, ...result });
          return;
        } catch {
          throw new PurchasingApiError("EDI_SUBMISSION_FAILED");
        }
      }
